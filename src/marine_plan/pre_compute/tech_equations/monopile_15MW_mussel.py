...
class monopile_15MW_mussel:
    def __init__(self, geo_data, NID_type=0, verbose=False):
        ...
        #unit density and interdistance
        self.unit_density_mussel = 36 #units per square km
        self.mussel_reduction = 0.7 #reduction in space due to maintenance routs and safety etc
        self.OPEX_reduction_mussel = 0.9 #OPEX reduction for solar because it can be combined with wind 
    ...
    def _calculations(self):
        ...
        #other costs
        pre_capex_mussel = (longline_cost_per_plot +
        ...
        precontruction_cost = 0.027 * pre_capex_mussel
        insurance_cost = 0.013 * pre_capex_mussel
        ...
        small_workboat_emissions = (small_workboat_fuel_transit + small_workboat_fuel_installation)*self.emission_factor_MDO*self.OPEX_reduction_mussel #kgCO2eq
        ...
        small_fast_boat_emissions = (small_fast_boat_fuel_transit + small_fast_boat_fuel_installation)*self.emission_factor_MDO*self.OPEX_reduction_mussel #kgCO2eq
        ...
        #total costs
        total_CAPEX_mussel = pre_capex_mussel + precontruction_cost + insurance_cost +large_workboat_cost + small_workboat_cost_instal_decom + total_dropper_cost
        total_OPEX_over_lifetime_mussel = (small_workboat_cost_operation + small_fast_boat_cost_operation)*self.lifetime_primary
        ...
        #-----------------------------------------------------------#
        #------------- Mussel change in final metrics --------------#
        #-----------------------------------------------------------#

        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
        #lifetime compensation! mussel farm is (around) half the lifetime of a windfarm. Do we assume that we reinstall the farms after 2 year?
        #then the everything is multiplied by 2

        lifetime_compensator_mussel = 2


        # STILL NEED TO DO TIMES THE UNIT DENSITY, THIS IS FOR ONE WHOLE TURBINE.
        self.final_metrics['capex'] = (total_CAPEX +
                                        (total_CAPEX_mussel*self.unit_density_mussel*self.mussel_reduction)*(1/self.unit_density)*lifetime_compensator_mussel) #adjust for mussel CAPEX
        
        self.final_metrics['opex'] = (total_OPEX_over_lifetime + 
                                      ((total_OPEX_over_lifetime_mussel*self.unit_density_mussel*self.mussel_reduction)*self.OPEX_reduction_mussel)*(1/self.unit_density)*lifetime_compensator_mussel) #adjust for mussel OPEX
        
        self.final_metrics['co2+'] = (total_emissions +
                                      (total_emissions_mussel*self.unit_density_mussel*self.mussel_reduction)*(1/self.unit_density)*lifetime_compensator_mussel) 
        ...
        self.final_metrics['revenue'] = (yearly_total_energy_produced*40 + 
                                        (yearly_food_production*3*self.unit_density_mussel*self.mussel_reduction)*(1/self.unit_density)) #40 being the general price (€/MWh) of energy + mussel revenue 
        ...
        NPV = NPV_calculator(total_CAPEX,
                            (total_CAPEX_mussel*self.unit_density_mussel*self.mussel_reduction)*(1/self.unit_density)*lifetime_compensator_mussel,
                            total_OPEX_over_lifetime,
                            ((total_OPEX_over_lifetime_mussel*self.unit_density_mussel*self.mussel_reduction)*self.OPEX_reduction_mussel)*(1/self.unit_density)*lifetime_compensator_mussel,
                            self.energy_revenue,
                            yearly_total_energy_produced,
                            (yearly_food_production*3*self.unit_density_mussel*self.mussel_reduction)*(1/self.unit_density),
                            self.food_revenue,
                            self.WACC,
                            self.WACC_food,
                            1,
