...
class spar_15MW_taut_driv:
    def __init__(self, geo_data, NID_type=0, verbose=False):
        ...
        self.tug_boat_fuel_transit = 739.7 #
        ...
        self.ahv_transit_speed = 2 #km/h
        self.ahv_day_rate = 150000 #€/day
        self.ahv_fuel_transit = 0 #
        self.ahv_fuel_installation = 0 #
    ...
    def _calculations(self):
        ...
        #dimensioning of spar
        stiffend_column_mass = 535.93+17.664*self.turbine_rating**2+ 0.02328*self.depth*math.log(self.depth)
        tapered_column_mass = 125.81*math.log(self.turbine_rating) +58.712
        ballast_mass = (-16.536*self.turbine_rating**2) +1261.8*self.turbine_rating - 1554.6
        secondary_steel_mass = math.exp(3.58 + 0.196*(self.turbine_rating**(1/2))*math.log(self.turbine_rating) + 0.00001*self.depth*math.log(self.depth))
            
        #economic variables
        auxiliary_systems_and_corrosion_protection = 1488000 #€
        stiffened_column_cost = stiffend_column_mass*2932.8 
        tapered_column_cost	= tapered_column_mass*3926
        ballast_cost = ballast_mass*93
        secondary_steel_cost = 6815*secondary_steel_mass	
        corossion_protection = 20_000_000*1.16/30
        total_floating_substructure_cost_for_one_turbine = auxiliary_systems_and_corrosion_protection + stiffened_column_cost + tapered_column_cost + ballast_cost + secondary_steel_cost + corossion_protection

        #emission variables
        stiffened_column_emissions	= 638*stiffend_column_mass
        tapered_column_emissions	= 3514*tapered_column_mass
        ballast_emissions	=  568*ballast_mass
        secondary_steel_emissions	= 3514*secondary_steel_mass
        total_floating_substructure_emissions_for_one_turbine	= stiffened_column_emissions + tapered_column_emissions + ballast_mass + secondary_steel_emissions
        ...
        towing_speed = 6 #km/s
        ahv_fuel_transit_tug = 0
        ahv_fuel_transit = 0
        ahv_fuel_installation = 0
        ...
        tow_to_site = self.distance_to_installation_port/towing_speed #h
            
        transit_back_to_shore = self.distance_to_installation_port/16 #h
        ...
        #emissions
        fuel_consumption_transit = (move_substructure_to_assembly + tow_to_site)*ahv_fuel_transit_tug + (transit_back_to_shore*ahv_fuel_transit)
        fuel_consumption_installation = (fixed_total_hours_per_structure - move_substructure_to_assembly)*ahv_fuel_installation
        emissions_spar_installation	= (fuel_consumption_transit + fuel_consumption_installation)*3*self.emission_factor_MDO
        
        #----------------------------------------------------------------------------------#

        #------------------------------ taut driven pile hardware ----------------------------#
        ...
        line_length = self.depth/math.cos(30*3.14/180)
        line_mass = line_length*line_mass_per_meter #tonnes
        driven_pile_mass = 124 #tonnes
        ...
        driven_pile_cost = drag_embedment_cost*4.4
       
        total_mooring_anchoring_catenary_per_turbine = (driven_pile_cost + line_cost)*amount_of_anchors #€
        
        #emission variables
        mooring_emissions = 1665
        driven_pile_emissions = 1799
        emissions_mooring_anchoring_catenary_per_turbine = (mooring_emissions*line_mass + driven_pile_emissions*driven_pile_mass)*amount_of_anchors
        ...
        driven_pile_install_multiplication_factor = 1.4

        #economic variable	
        anchor_installation_cost = (((total_anchor_installation_time/24)*(ahv_day_rate + additional_anchor_equipment_cost))*weather_factor)*driven_pile_install_multiplication_factor
    
        #emission variable
        fuel_consumption_transit_anchor = (transit_to_anchor + transit_to_harbor + transit_to_next_anchor)*ahv_fuel_transit/100
        fuel_consumption_installation_anchor = (total_anchor_installation_time - (transit_to_anchor - transit_to_next_anchor - transit_to_harbor)/100)*ahv_fuel_installation*driven_pile_install_multiplication_factor
        ...
        yearly_OenM_for_one_turbine	= OPEX_cost_per_turbine_inspection + OPEX_cost_per_turbine_cable_inspection + OPEX_cost_per_turbine_substation_inspection + OPEX_cost_per_turbine_heavy + OPEX_cost_per_turbine_large + OPEX_cost_per_turbine_small
        ...
        #emission variable
        floating_wind_decom_emissions = (emissions_spar_installation + emissions_cable_installation + emissions_anchor_installation)*floating_wind_decom_factor
